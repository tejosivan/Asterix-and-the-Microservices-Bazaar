    ...
    with requests.Session() as session:
        for i in range(5):
            ...
            while True:
                ...
                endTime = time.time()
                lookup_latencies.append(endTime - startTime)
                if lookup_response.status_code == 200:
                    ...
                    if quantity > 0:
                        ...
                        endTime = time.time()
                        trade_latencies.append(endTime - startTime)
        ...
        for trade in trades_record:
            ...
            endTime = time.time()
            order_latencies.append(endTime - startTime)
...
if __name__ == "__main__":
    ...
    lookup_latency = sum(lookup_latencies) / len(lookup_latencies)
    trade_latency = sum(trade_latencies) / len(trade_latencies)
    order_latency = sum(order_latencies) / len(order_latencies)
    ...
    with open(latencydumpfile, "w") as f:
        ...
        f.write("\n\n\n")
        f.write("lookup latencies:\n")
        for i in lookup_latencies:
            f.write(str(i) + "\n")
        f.write("trade latencies:\n")
        for i in trade_latencies:
            f.write(str(i) + "\n")
